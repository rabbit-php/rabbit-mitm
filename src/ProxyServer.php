    ...
    protected function startServer($server = null): void
    {
        ...
        $server->handle(function (Connection $conn) {
            ...
            while (strlen((string)$data = $conn->recv()) > 0) {
                ...
                fwrite($remote, $data);
                $buffer = new Buffer(0);
                !$send && rgo(function () use ($buffer, $remote, $socket, $request, $host, $mitm, &$send) {
                    $send = true;
                    while ($send) {
                        $block = true;
                        stream_set_blocking($remote, true);
                        while (strlen((string) ($tmp = fread($remote, 2048))) > 0) {
                            if ($block) {
                                $block = false;
                    ...
                    fclose($remote);
                    if ($mitm) {
                        $response = new \http\Message($buffer->toString());
                        $buffer->clear();
                        $this->handlers[$host]($request, $response);
                    }
                });
            }
            $send = false;
